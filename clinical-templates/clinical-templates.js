  ...
  templateEditor.innerHTML = `
    <div class="es-template-meta">
      ...
      <div class="es-check-grid">
        ...
        <label class="es-check-option">
          <input id="templateClientSendable" type="checkbox" ${activeTemplate.is_client_sendable === 1 ? "checked" : ""}>
          Client can complete this form
        </label>
      </div>

      <p class="es-muted-copy">
        Client-sendable forms can be assigned to services and sent securely to customers. Leave this off for staff-only records such as patch tests and treatment records.
      </p>
    </div>
...
function bindEditorEvents() {
  ...
  document.getElementById("templateClientSendable").addEventListener("change", event => {
    activeTemplate.is_client_sendable = event.target.checked ? 1 : 0;
  });
