...
function money(v,fallback=null){const t=clean(v,40).replace(/[£$€\s,]/g,"").replace(/^(GBP|USD|EUR|AUD|NZD|ZAR)/i,"");if(!t)return fallback;const n=Number(t);return Number.isFinite(n)&&n>=0?Math.round(n*100):null;}
...
function noteWithDiscount(notes, discountMinor, voucher, currency="GBP"){const parts=[clean(notes,3000)];if(discountMinor>0)parts.push(`Imported discount ${currency} ${(discountMinor/100).toFixed(2)}${voucher?` · voucher ${voucher}`:""}`);else if(voucher)parts.push(`Imported voucher ${voucher}`);return parts.filter(Boolean).join(" · ")||null;}
...
function sourceRef(system,id){const sid=clean(id,240);if(!sid)return"";const source=clean(system,80)||"external";return `${source}:${sid}`;}
function paymentDateTime(v){const d=dateOnly(v);return d?`${d}T12:00:00`:null;}

export async function onRequestGet({request,env}){
  const user=await getUserContext(request,env);if(!user)return unauthorized();
  const [services, templates, variants, staff] = await Promise.all([
    ...
    env.DB.prepare(`SELECT id,name,email FROM users WHERE business_id=? AND is_active=1 ORDER BY name COLLATE NOCASE`).bind(user.business_id).all()
  ]);
  return Response.json({ok:true,max_rows:MAX_ROWS,services:services.results||[],package_templates:templates.results||[],package_variants:variants.results||[],staff:staff.results||[]});
}
...
export async function onRequestPost({request,env}){
 try{
  ...
  const [serviceRes,customerRes,refRes,voucherSetting,templateRes,variantRes,existingPackageRes,existingConsultationRes,staffRes]=await Promise.all([
    ...
    env.DB.prepare(`SELECT id,name,email FROM users WHERE business_id=? AND is_active=1`).bind(user.business_id).all()
  ]);
  const serviceRows=serviceRes.results||[];const services=new Map(serviceRows.map(s=>[key(s.name),s]));const servicesById=new Map(serviceRows.map(s=>[s.id,s]));const staffRows=staffRes.results||[];const staffByName=new Map(staffRows.map(u=>[key(u.name),u]));const staffByEmail=new Map(staffRows.map(u=>[key(u.email),u]));
  ...
  for(const input of rows.Customers){
    ...
    const customerExternal=sourceRef(input.source_system,input.source_client_id);
    let customer=null;if(customerExternal&&existingRefs.has(`customer:${key(customerExternal)}`))customer=customersById.get(existingRefs.get(`customer:${key(customerExternal)}`))||null;if(!customer&&email)customer=customersByEmail.get(key(email));if(!customer&&ph)customer=customersByPhone.get(phoneKey(ph));
    ...
    if(customer&&customerExternal&&!existingRefs.has(`customer:${key(customerExternal)}`)){statements.push(env.DB.prepare(`INSERT INTO data_import_references(business_id,import_batch_id,entity_type,external_reference,internal_id) VALUES(?,?,?,?,?)`).bind(user.business_id,batchId,"customer",customerExternal,customer.id));existingRefs.set(`customer:${key(customerExternal)}`,customer.id);}
  }
  ...
  for(const input of rows.Bookings){
    ...
    if(customer&&service?.service_type==="consultation"&&status==="completed"&&date&&time&&paid!==null&&paid>0){
      const external=sourceRef(input.source_system,input.source_booking_id)||autoRef("booking",[customer.id,service.id,date,time]);
  ...
  for(const input of rows.Packages){
    ...
    if(clean(input.start_date)&&!starts)err(errors,"Packages & Courses",rn,"start_date","Start date is invalid.");if(clean(input.expiry_date)&&!expires)err(errors,"Packages & Courses",rn,"expiry_date","Expiry date is invalid.");if(clean(input.payment_date)&&!paidAt)err(errors,"Packages & Courses",rn,"payment_date","Payment date is invalid.");
    ...
    if(customerId&&choice&&consultationCredit>0){
      ...
      else if(consultationCredit>creditSource.paidMinor)err(errors,"Packages & Courses",rn,"consultation_credit",`Consultation credit cannot exceed the consultation payment of ${String(user.currency||"GBP").toUpperCase()} ${(creditSource.paidMinor/100).toFixed(2)}.`);
    }
    if(customerId&&choice&&PACKAGE_STATUSES.has(status)&&finalPrice!==null&&consultationCredit!==null&&remainingAfterCredit!==null&&paid!==null&&paid<=remainingAfterCredit&&(!consultationCredit||creditSource)){
      ...
      const external=sourceRef(input.source_system,input.source_package_id)||autoRef("package",[customerId,choice.variant?.id||choice.template.id,starts||"",expires||"",finalPrice]);
      ...
      if(paid>0){const pid=`pay_${crypto.randomUUID()}`,ptype=paid+consultationCredit>=finalPrice?"full":"deposit",amountBeforeDiscount=Math.max(0,basePrice-consultationCredit),providerRef=paymentRef||`import:${external}`;packagePaymentId=pid;paymentsCreated++;statements.push(env.DB.prepare(`INSERT INTO payments(id,business_id,appointment_id,customer_id,provider,payment_type,amount_minor,currency,status,provider_reference,paid_at,payment_method,notes) VALUES(?,?,NULL,?,'manual',?,?,?,'paid',?,?,?,?)`).bind(pid,user.business_id,customerId,ptype,paid,String(user.currency||"GBP").toUpperCase(),providerRef,paidAt||new Date().toISOString().slice(0,19),method,discountPaymentNotes("Imported historical package payment",discount,voucher,amountBeforeDiscount,currentVouchers)));statements.push(env.DB.prepare(`INSERT INTO customer_package_payments(customer_package_id,payment_id) VALUES(?,?)`).bind(id,pid));if(paymentRef&&!existingRefs.has(`payment:${key(paymentRef)}`)){statements.push(env.DB.prepare(`INSERT INTO data_import_references(business_id,import_batch_id,entity_type,external_reference,internal_id) VALUES(?,?,?,?,?)`).bind(user.business_id,batchId,"payment",paymentRef,pid));existingRefs.set(`payment:${key(paymentRef)}`,pid);}}
  ...
  for(const input of rows.Bookings){
    const rn=Number(input.__row||0),customerId=resolveCustomer(input.customer_email_or_phone,"Bookings",rn),serviceName=clean(input.eselram_service,200),service=services.get(key(serviceName)),date=dateOnly(input.appointment_date),time=timeOnly(input.start_time),endTime=clean(input.end_time)?timeOnly(input.end_time):null,durationInput=Number(clean(input.duration_minutes,20)||0),status=normalizedStatus(input.status,"confirmed"),packageLabel=clean(input.package_or_course,250),pkg=customerId&&packageLabel?findPackageForBooking(customerId,packageLabel):null,discount=money(input.discount,0),voucher=clean(input.voucher_used,80).toUpperCase(),paid=money(input.amount_already_paid,0),method=clean(input.payment_method,80)||"imported",paidAt=clean(input.payment_date)?paymentDateTime(input.payment_date):null,paymentRef=clean(input.payment_reference,240),practitionerRaw=clean(input.practitioner,254),practitioner=practitionerRaw?(staffByEmail.get(key(practitionerRaw))||staffByName.get(key(practitionerRaw))):null;
    if(clean(input.end_time)&&!endTime)err(errors,"Bookings",rn,"end_time","End time is invalid.");if(clean(input.duration_minutes)&&(!Number.isFinite(durationInput)||durationInput<=0))err(errors,"Bookings",rn,"duration_minutes","Duration must be a positive number of minutes.");if(clean(input.payment_date)&&!paidAt)err(errors,"Bookings",rn,"payment_date","Payment date is invalid.");if(!service)err(errors,"Bookings",rn,"eselram_service",`'${serviceName||"blank"}' does not match an active Eselram service. Choose a value from the dropdown.`);if(!date)err(errors,"Bookings",rn,"appointment_date","Appointment date is invalid.");if(!time)err(errors,"Bookings",rn,"start_time","Start time is invalid.");if(!BOOKING_STATUSES.has(status))err(errors,"Bookings",rn,"status","Status must be Confirmed, Completed, Cancelled or No show.");
    ...
    if(customerId&&service&&date&&time&&BOOKING_STATUSES.has(status)&&finalPrice!==null&&paid!==null&&paid<=finalPrice&&(!packageLabel||pkg)){
      const external=sourceRef(input.source_system,input.source_booking_id)||autoRef("booking",[customerId,service.id,date,time]);if(existingRefs.has(`booking:${key(external)}`)){err(errors,"Bookings",rn,"appointment_date","This booking appears to have already been imported.");continue;}
      const id=plannedBookingIds.get(input)||`apt_${crypto.randomUUID()}`,duration=durationInput>0?Math.round(durationInput):Number(service.duration_minutes||30),start=`${date}T${time}:00`,end=endTime?`${date}T${endTime}:00`:addMinutes(date,time,duration),kind=service.service_type==="consultation"?"consultation":"service",internalNotes=[noteWithDiscount(null,discount,voucher),practitionerRaw&&!practitioner?`Imported practitioner: ${practitionerRaw}`:""].filter(Boolean).join(" · ")||null;
      statements.push(env.DB.prepare(`INSERT INTO appointments(id,business_id,customer_id,service_id,assigned_user_id,status,start_at,end_at,price_minor,deposit_due_minor,booking_source,booking_kind,customer_notes,internal_notes,cancelled_at,cancellation_reason,import_batch_id,external_reference,reminders_enabled) VALUES(?,?,?,?,?,?,?,?,?,0,'import',?,?,?,CASE WHEN ?='cancelled' THEN CURRENT_TIMESTAMP ELSE NULL END,?,?,?,?)`).bind(id,user.business_id,customerId,service.id,practitioner?.id||null,status,start,end,finalPrice,kind,clean(input.notes,3000)||null,internalNotes,status,clean(input.cancellation_reason,500)||null,batchId,external,yesNo(input.send_future_reminder,false)?1:0));
      ...
      if(paid>0){const pid=`pay_${crypto.randomUUID()}`,ptype=paid>=finalPrice?"full":"deposit",providerRef=paymentRef||`import:${external}`;paymentsCreated++;statements.push(env.DB.prepare(`INSERT INTO payments(id,business_id,appointment_id,customer_id,provider,payment_type,amount_minor,currency,status,provider_reference,paid_at,payment_method,notes) VALUES(?,?,?,?,'manual',?,?,?,'paid',?,?,?,?)`).bind(pid,user.business_id,id,customerId,ptype,paid,String(user.currency||"GBP").toUpperCase(),providerRef,paidAt||new Date().toISOString().slice(0,19),method,"Imported historical booking payment"));if(paymentRef&&!existingRefs.has(`payment:${key(paymentRef)}`)){statements.push(env.DB.prepare(`INSERT INTO data_import_references(business_id,import_batch_id,entity_type,external_reference,internal_id) VALUES(?,?,?,?,?)`).bind(user.business_id,batchId,"payment",paymentRef,pid));existingRefs.set(`payment:${key(paymentRef)}`,pid);}}
    }
  }
  ...
  for(const input of rows.Treatments){
    ...
    if(customerId&&service&&TREATMENT_STATUSES.has(status)&&date){const external=sourceRef(input.source_system,input.source_treatment_id)||autoRef("treatment",[customerId,service.id,date,clean(input.practitioner,200),clean(input.treatment_area,300)]);if(existingRefs.has(`treatment_record:${key(external)}`)){err(errors,"Treatment History",rn,"treatment_date","This treatment record appears to have already been imported.");continue;}const id=`tr_${crypto.randomUUID()}`;statements.push(env.DB.prepare(`INSERT INTO treatment_records(id,business_id,appointment_id,customer_id,service_id,status,treatment_date,practitioner_name,treatment_area,device_name,device_settings,treatment_notes,client_response,client_tolerance,aftercare_notes,next_session_plan,next_treatment_date,import_batch_id,external_reference) VALUES(?,?,NULL,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(id,user.business_id,customerId,service.id,status,date,clean(input.practitioner,200)||null,clean(input.treatment_area,500)||null,clean(input.device,300)||null,[clean(input.device_settings,3000),clean(input.wavelength,120)?`Wavelength: ${clean(input.wavelength,120)}`:"",clean(input.fluence,120)?`Fluence: ${clean(input.fluence,120)}`:""].filter(Boolean).join(" · ")||null,[clean(input.treatment_notes,5000),clean(input.session_number,80)?`Imported session number: ${clean(input.session_number,80)}`:""].filter(Boolean).join(" · ")||null,clean(input.client_response,3000)||null,clean(input.client_tolerance,1000)||null,clean(input.aftercare_notes,3000)||null,clean(input.next_session_plan,3000)||null,nextDate,batchId,external));statements.push(env.DB.prepare(`INSERT INTO data_import_references(business_id,import_batch_id,entity_type,external_reference,internal_id) VALUES(?,?,?,?,?)`).bind(user.business_id,batchId,"treatment_record",external,id));}
  }
