...
export async function installedBillingAssertion(env, action, returnUrl = "") {
  ...
  const billingAction = String(action || "").trim().toLowerCase();
  const safeReturnUrl = String(returnUrl || "").trim();
  if (!installationId || !currentVersion || !secret) {
    const error = new Error("Subscription management will become available after this installation receives its secure Eselram handoff configuration.");
    error.status = 409;
    throw error;
  }
  if (!["status", "cancel", "resume", "payment-method"].includes(billingAction)) {
    const error = new Error("Unsupported subscription management action.");
    error.status = 400;
    throw error;
  }
  const timestamp = Date.now();
  const nonce = randomNonce();
  const message = [installationId, currentVersion, String(timestamp), nonce, billingAction, safeReturnUrl].join("\n");
  ...
  return {
    ...
    return_url: safeReturnUrl || undefined,
    signature
  };
}
...
export function subscriptionBrokerBase(request, env) {
  try {
    const host = new URL(request.url).hostname.toLowerCase();
    // QA-only override: permanent Eselram QA must use the version-specific
    // Provisioner candidate while subscription billing is being validated.
    // Production installations continue to use the normal configured broker.
    if (host === "eselram-qa-admin.eselram.com") {
      return "https://32850357-eselram-provisioner.mroschhaden.workers.dev";
    }
  } catch {}
  return updateBrokerBase(env);
}

export async function brokerJson(env, path, payload, brokerBase = "") {
  const base = String(brokerBase || updateBrokerBase(env)).replace(/\/$/, "");
