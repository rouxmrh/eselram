...
/**
 * Package-specific deduction calculation.
 *
 * For percentage discounts (including percentage vouchers), the percentage is
 * calculated from the package value before consultation credit. Fixed-amount
 * deductions keep their existing behaviour and are calculated against the
 * currently payable balance.
 */
export async function calculatePackagePaymentDeduction({
  env,
  businessId,
  packagePriceMinor,
  payableBaseMinor,
  ...
  const payableBase = Math.max(0, Math.round(Number(payableBaseMinor || 0)));

  const initial = await calculatePaymentDeduction({
    env,
    businessId,
    baseAmountMinor: payableBase,
    deduction
  });

  const isPercentage =
    initial.type === "percent" ||
    (initial.type === "voucher" && initial.voucher?.discount_type === "percent");

  if (!isPercentage) return initial;

  const packageBase = Math.max(0, Math.round(Number(packagePriceMinor || 0)));
  if (!packageBase || packageBase === payableBase) return initial;

  return calculatePaymentDeduction({
    env,
    businessId,
    baseAmountMinor: packageBase,
    deduction
  });
}
