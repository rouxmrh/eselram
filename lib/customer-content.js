...
const BOOKING_PATCH_TEST_SETTING_KEY =
  "communications.public_booking_patch_test_copy";
...
export const PUBLIC_BOOKING_PATCH_TEST_DEFAULT =
  "A patch test is required before the first treatment. The business will confirm the patch-test requirements with you.";

export const EMAIL_VARIABLE_RULES = {
  booking_confirmation: { allowed: ["customer_name","business_name","service_name","default_closing"], required: { intro: ["customer_name","business_name"] } },
  appointment_reminder: { allowed: ["customer_name","business_name","service_name"], required: { intro: ["customer_name","business_name"] } },
  cancellation_confirmation: { allowed: ["customer_name","business_name","service_name"], required: { intro: ["customer_name","business_name"] } },
  reschedule_confirmation: { allowed: ["customer_name","business_name","service_name"], required: { intro: ["customer_name","business_name"] } },
  payment_receipt: { allowed: ["customer_name","business_name","service_name","amount","default_subject","default_title"], required: { subject: ["default_subject"], title: ["default_title"], intro: ["customer_name"] } },
  package_payment_confirmation: { allowed: ["customer_name","business_name","service_name","amount","default_subject","default_title"], required: { subject: ["default_subject"], title: ["default_title"], intro: ["customer_name"] } },
  payment_link: { allowed: ["customer_name","business_name","service_name","amount"], required: { intro: ["customer_name","business_name","service_name"] } },
  client_form_request: { allowed: ["customer_name","business_name","service_name","form_name"], required: { subject: ["business_name","form_name"], intro: ["business_name","form_name"] } },
  client_form_reminder: { allowed: ["customer_name","business_name","service_name","form_name"], required: { subject: ["business_name","form_name"], intro: ["business_name","form_name"] } }
};
...
async function getTextMapSetting(env, businessId, key) {
  const raw = parseJson(await getSetting(env, businessId, key), {});
  const result = {};
  for (const [mapKey, value] of Object.entries(raw)) {
    const cleanKey = cleanText(mapKey, 160);
    const text = cleanText(value, 4000);
    if (cleanKey && text) result[cleanKey] = text;
  }
  return result;
}

export async function getPublicBookingCopyOverrides(env, businessId) {
  return await getTextMapSetting(env, businessId, BOOKING_SETTING_KEY);
}

export async function getPublicBookingPatchTestCopyOverrides(env, businessId) {
  return await getTextMapSetting(env, businessId, BOOKING_PATCH_TEST_SETTING_KEY);
}
...
export function publicBookingPatchTestSettingKey() {
  return BOOKING_PATCH_TEST_SETTING_KEY;
}
