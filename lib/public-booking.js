export function publicRequestOrigin(request) {
  ...
  // When the public page is being served on a branded Eselram hostname, the
  // browser calls the buyer-owned pages.dev API directly. Use the browser
  // Origin for customer-facing redirects, Stripe returns and generated links.
  const candidates = [
    request.headers.get("Origin"),
    request.headers.get("X-Eselram-Public-Origin")
  ];
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      // Ignore malformed origins and continue to the installation origin.
    }
  }
...
export async function cleanupPendingPublicPackageSales(env, businessId) {
  // Public package purchases are provisional until Stripe confirms payment.
  // Never expose or retain abandoned pending checkouts as real finance records.
  const stale = await env.DB
    .prepare(`
      SELECT
        ps.id AS sale_id,
  ...
  const rows = Array.isArray(stale?.results) ? stale.results : [];
  ...
  for (const row of rows) {
    ...
    // Remove the technical sale first, then its unpaid technical payment.
    ...
    if (customerId && Number(row.customer_created_for_checkout || 0) === 1) {
      await deleteUnusedCustomer(env, businessId, customerId);
    }
  }
}
