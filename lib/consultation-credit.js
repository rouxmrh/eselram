...
export async function findAvailableConsultationCredit({
  ...
  for (const row of rows.results || []) {
    const used = await env.DB.prepare(`
      ...
      LEFT JOIN customer_packages package
        ON package.id = sale.customer_package_id
       AND package.business_id = sale.business_id
      WHERE
        sale.business_id = ?
        AND sale.consultation_credit_source_appointment_id = ?
        AND sale.status = 'paid'
        AND (
          sale.customer_package_id IS NULL
          OR package.status IS NULL
          OR package.status != 'cancelled'
        )
