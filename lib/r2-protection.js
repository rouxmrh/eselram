...
export async function preserveR2ObjectBeforeDelete({
  ...
  mimeType = null, sizeBytes = null, reason = "delete"
}) {
  ...
  try {
    await env.DB.prepare(`
      INSERT INTO eselram_file_recovery_objects (
        id,business_id,original_key,recovery_key,source_type,source_id,original_name,mime_type,size_bytes,reason,status,protected_until
      ) VALUES (?,?,?,?,?,?,?,?,?,?,'available',?)
    `).bind(id,businessId,originalKey,recoveryKey,sourceType || 'file',sourceId,originalName,contentType,bytes,reason,protectedUntil).run();
  } catch (error) {
...
export async function restoreProtectedR2Object({ env, businessId, recoveryId }) {
  ...
  await env.FORM_UPLOADS.put(row.original_key, object.body, { httpMetadata: object.httpMetadata || undefined });
  await env.DB.prepare(`UPDATE eselram_file_recovery_objects SET status='restored', restored_at=CURRENT_TIMESTAMP WHERE id=? AND business_id=?`).bind(recoveryId,businessId).run();
  return row;
}
