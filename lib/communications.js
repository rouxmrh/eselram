...
export function genericAftercareTemplate(
  ...
  return {
    ...
    sections: [
      [
        "After Treatment",
        [
          "Follow the aftercare advice provided by your practitioner.",
          "Keep the treated area clean and avoid unnecessary touching.",
          "Use only products your practitioner has advised are suitable."
        ]
      ],
      [
        "What to Avoid",
        [
          "Avoid activities or products your practitioner has advised against while the area settles.",
          "Do not pick, scratch or irritate the treated area."
        ]
      ],
      [
        "When to Contact Your Practitioner",
        [
          "Contact the business if you have any concerns about your recovery or reaction after treatment."
        ]
      ]
    ],
    note:
      "These are general aftercare instructions. Your practitioner may provide additional advice specific to your treatment."
  };
}


function serviceAftercareSettingKey(
  serviceId
) {
  return `communications.aftercare.service.${serviceId}`;
}


function groupAftercareSettingKey(
  groupName
) {
  const safe =
    encodeURIComponent(
  ...
  return `communications.aftercare.group.${safe}`;
}


async function getGroupAftercareSetting(
  env,
  businessId,
  groupName
) {
...
async function getServiceAftercareSetting(
  env,
  businessId,
  serviceId
) {
...
export async function getBusinessAftercareForService(
  env,
  businessId,
  serviceId,
  serviceName,
  ...
  /*
   * New commercial model: one aftercare template per main service group.
   * Package/service variants inherit the same template automatically.
   */
  const groupSetting =
    await getGroupAftercareSetting(
      env,
      businessId,
      groupName
    );
  ...
  if (groupSetting) {
    ...
    return normaliseAftercareTemplate(
      groupSetting.template ||
      groupSetting,
      fallback
    );
  }
  ...
  /*
   * Backward compatibility with the immediately previous service-specific
   * implementation. Existing saved settings still work until the business
   * next saves the group-level version in Communications.
   */
  const serviceSetting =
    await getServiceAftercareSetting(
      env,
      businessId,
      serviceId
    );
  ...
  /*
   * Preserve the original Eselram starter defaults.
   * Custom/new service groups remain OFF until explicitly enabled.
   */
  const starter =
    aftercareContentFor(
      groupName
...
export function groupAftercareKey(
  groupName
) {
  return groupAftercareSettingKey(
    groupName
  );
}


export function serviceAftercareKey(
  serviceId
) {
  return serviceAftercareSettingKey(
    serviceId
  );
}
...
function aftercareTemplateFor({
  ...
  /*
   * The service resolver is the source of truth here.
   * A null value intentionally means aftercare is disabled for this service.
   */
  const content =
    aftercareContent;
  ...
  const logo =
    logoUrl
      ? `<img src="${escapeHtml(
          ...
          appointment.business_name
        )}" style="display:block;max-width:180px;max-height:64px;width:auto;height:auto;margin:0 0 20px;">`
      : "";
...
export async function sendAppointmentCommunication({
  ...
  const emailLogoUrl =
    ...
    appointment.logo_data_url
      ? `${resolvedBaseUrl}/api/public-branding/logo`
      : null;
  ...
  if (
    ...
    if (isConsultation) {
      return {
        ok: true,
        skipped: true,
        reason: "consultation_not_treatment"
      };
    }
  }

  const aftercareServiceName =
    String(
      appointment.service_name ||
      ""
    );
  ...
  const aftercareContent =
    type === "treatment_aftercare"
      ? await getBusinessAftercareForService(
          ...
          aftercareServiceName,
          appointment.booking_group
        )
      : null;
  ...
  const detailRows =
    isPackage
      ? [
          ...
          ...(Number(row.discount_minor || 0) > 0
            ? [["Deduction applied", money(Number(row.discount_minor || 0), row.currency || "GBP")]]
            : []),
  ...
  try {
    ...
    if (
      ...
      if (configuredBaseUrl) {
        const logoUrl =
          `${configuredBaseUrl}/api/public-branding/logo`;

        paymentLogoHtml =
          `<img src="${escapeHtml(logoUrl)}" alt="${escapeHtml(row.business_name)}" style="display:block;max-width:180px;max-height:64px;width:auto;height:auto;margin:0 0 20px;">`;
      }
    }
  } catch (brandingError) {
    console.error(
      "Payment confirmation logo lookup failed; sending email without logo:",
      brandingError
    );
  }
...
export async function sendPaymentLinkEmail({
  ...
  if (!row) {
    return {
      ok: false,
      error:
        "The payment link could not be matched to this booking or package sale."
    };
  }
    ...
    <div style="margin:0;padding:32px;background:${escapeHtml(
      ...
      <div style="max-width:560px;margin:0 auto;background:${escapeHtml(
        ...
        <h1 style="margin:0 0 18px;font-size:26px">
          ${escapeHtml(
            paymentLinkTemplate.title
          )}
        </h1>

        <p>
          ${escapeHtml(
            paymentLinkTemplate.intro
          )}
        </p>
        ...
        <p style="margin:0 0 18px;font-size:14px;line-height:1.5">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            style="color:${escapeHtml(primary)};text-decoration:underline;font-weight:600"
          >
