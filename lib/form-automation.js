...
function buildHtml(row,formUrl,logoUrl=null) {
...
export async function runServiceFormAutomation({env,businessId,appointmentId,triggerEvent,baseUrl,createdByUserId=null}) {
  ...
  for (const rule of selectedRules) {
    ...
    if (String(rule.template_type||'').toLowerCase()==='consultation') {
      const completedConsultation=await env.DB.prepare(`
        SELECT r.id,r.submission_id,r.submitted_at
        FROM clinical_form_requests r
        ...
        ORDER BY datetime(COALESCE(r.submitted_at,r.created_at)) DESC
        LIMIT 1
      `).bind(businessId,appointment.customer_id,appointment.service_id).first();
