...
function isInsufficientGmailScopeError(value) {
...
async function markGmailReconnectRequired(env, businessId, message) {
  try {
    await env.DB
      .prepare(`
...
async function gmailConnection(env, businessId) {
  ...
  const permissionRequired = Boolean(
    ...
    !hasRequiredGmailScope(credentials.scope)
  );
...
export async function getActiveEmailConnection(
  ...
  // Central Eselram Email is now the primary transactional email provider.
  // Treat it as the active/ready connection before checking legacy
  // per-business Gmail or Resend integrations.
  const centralEmailApiUrl =
    String(env.ESELRAM_EMAIL_API_URL || "").trim();
  const centralEmailApiSecret =
    String(env.ESELRAM_EMAIL_API_SECRET || "").trim();

  if (centralEmailApiUrl && centralEmailApiSecret) {
    return {
      provider: "eselram",
      status: "configured",
      ready: true
  ...
  if (active === "gmail") {
    ...
    if (gmail?.permissionRequired) {
      return {
        error:
          "Email permission required. Reconnect Google in Settings → Email provider and allow Eselram to send email on your behalf."
      };
    }
  ...
  return {
    error:
      "Automated email is not ready. Connect Gmail in Settings → Email provider."
  };
}
...
export async function sendBusinessEmail(
  ...
  const centralEmailApiUrl =
    String(env.ESELRAM_EMAIL_API_URL || "")
      .trim()
      .replace(/\/+$/, "");
  ...
  // Prefer Eselram's central transactional email service when configured.
  // The legacy Gmail / per-business Resend path below remains available as
  // a rollback fallback while the central service is being proven in QA.
  if (centralEmailApiUrl && centralEmailApiSecret) {
    if (!businessReply) {
      throw new Error(
        "Business email is missing. Add a business email address before sending email."
      );
    }

    const response = await fetch(
      `${centralEmailApiUrl}/v1/send`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${centralEmailApiSecret}`,
          "Content-Type": "application/json",
          Accept: "application/json"
        },
        body: JSON.stringify({
    ...
    if (!response.ok || !data?.ok) {
      throw new Error(
        data?.error ||
        "Eselram email service rejected the email."
      );
    }
  ...
  if (connection.provider === "gmail") {
    ...
    if (!response.ok) {
      ...
      if (isInsufficientGmailScopeError(gmailError)) {
